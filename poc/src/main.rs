...
use vm::{
    IndexKind,
    file_format::{
        CompiledModule, CompiledScript, ModuleHandleIndex, TableIndex, StructDefinitionIndex,
...
impl Document {
    ...
    fn create_main_section(&mut self) {
        ...
        match doc_main {
            Section::Unresolved => {
                *doc_main = Section::Module(ModuleSec {
                    ..Default::default()
                });
            }
            Section::Module { .. } => {}
...
fn _describe_module(m: &CompiledModule) {
    ...
    println!(
        "FunctionHandle: {}",
        m.kind_count(IndexKind::FunctionHandle)
    );
