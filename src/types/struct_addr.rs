...
impl StructAddr {
	...
	pub fn addr(&self) -> &ModAddr { &self.0 }

	pub fn split(self) -> (ModAddr, String) { (self.0, self.1) }
	pub fn split_ref(&self) -> (&ModAddr, &str) { (&self.0, &self.1) }
}
...
impl<T: Into<StructAddr>> IntoResAddr for T {
	fn into_fn_addr(self) -> StructAddr { self.into() }
}

impl<'a, S: ToString> From<(ModAddr, S)> for StructAddr {
	fn from(v: (ModAddr, S)) -> Self { Self(v.0, v.1.to_string()) }
}
impl<'a, S: ToString> From<&'a (ModAddr, S)> for StructAddr {
	fn from(v: &'a (ModAddr, S)) -> Self { Self(v.0.to_owned(), v.1.to_string()) }
}
impl<'a, S: ToString> From<(&'a ModAddr, S)> for StructAddr {
	fn from(v: (&'a ModAddr, S)) -> Self { Self(v.0.to_owned(), v.1.to_string()) }
}
impl<'a, S: ToString> From<&'a (&'a ModAddr, S)> for StructAddr {
	fn from(v: &'a (&'a ModAddr, S)) -> Self { Self(v.0.to_owned(), v.1.to_string()) }
}
