...
impl ModAddr {
	...
	pub fn addr(&self) -> &AccountAddress { &self.0 }

	pub fn split(self) -> (AccountAddress, String) { (self.0, self.1) }
	pub fn split_ref(&self) -> (&AccountAddress, &str) { (&self.0, &self.1) }
}
...
impl<'a, S: ToString> From<&'a (AccountAddress, S)> for ModAddr {
	fn from(v: &'a (AccountAddress, S)) -> Self { Self(v.0, v.1.to_string()) }
}
impl<'a, S: ToString> From<(&'a AccountAddress, S)> for ModAddr {
	fn from(v: (&'a AccountAddress, S)) -> Self { Self(v.0.to_owned(), v.1.to_string()) }
}
...
impl From<libra::move_core_types::language_storage::ModuleId> for ModAddr {
	fn from(mod_id: ModuleId) -> Self { (mod_id.address(), mod_id.name().as_str()).into() }
}
...
impl UpperHex for ModAddr {
	fn fmt(&self, f: &mut Formatter<'_>) -> Result {
		UpperHex::fmt(&u128::from_be_bytes(self.0.into()), f).and_then(|_| write!(f, "::{}", self.1))
	}
}
...
mod tests {
	...
	fn addr_empty() -> ModAddr { (AccountAddress::new([0; 16]), "Foo".to_owned()).into() }
	fn addr_42() -> ModAddr {
		(AccountAddress::new(0x004200000000000_u128.to_be_bytes()), "Foo".to_owned()).into()
	}

	#[test]
	fn mod_addr_fmt_hex() {
		let addr: ModAddr = addr_42();
		assert_eq!("00000000000000000004200000000000::Foo", format!("{:x}", addr));
		assert_eq!("0x4200000000000::Foo", format!("{:#X}", addr));

		let addr: ModAddr = addr_empty();
		assert_eq!("00000000000000000000000000000000::Foo", format!("{:x}", addr));
