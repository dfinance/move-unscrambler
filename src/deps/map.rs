...
impl AsMap<ModAddr, ModInfo> for ModMap {
	fn as_map(&self) -> &HashMap<ModAddr, ModInfo> { self }
	fn as_map_mut(&mut self) -> &mut HashMap<ModAddr, ModInfo> { self }
}
...
impl ModInfo {
	...
	pub fn mod_addr(&self) -> ModAddr { (self.address(), self.name()).into_mod_addr() }
	pub fn bytecode(&self) -> &CompiledModule { &self.bytecode }
	pub fn dependencies(&self) -> &[ModAddr] { &self.dependencies[..] }
	pub fn dependencies_mut(&mut self) -> &mut [ModAddr] { &mut self.dependencies[..] }
}
