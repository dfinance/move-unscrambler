...
impl<Q> Into<Box<dyn DependencySearch<Q>>> for OnlineDependencySearch<String> where Q: Into<ModAddr> {
	fn into(self) -> Box<dyn DependencySearch<Q>> { Box::new(self) }
}

impl<Q> Into<Box<dyn DependencySearch<Q>>> for OnlineDependencySearch<&'static str> where Q: Into<ModAddr> {
	fn into(self) -> Box<dyn DependencySearch<Q>> { Box::new(self) }
}
